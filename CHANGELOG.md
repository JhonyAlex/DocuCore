...
- **Sistema multi-proyecto real (PROJ-01)**: `projectId` pasa a ser el ámbito autoritativo de las rutas operativas y de API. `ProjectScope` central valida proyecto, membresía, roles, archivo y referencias cruzadas; el Sidebar cambia entre proyectos conservando sección y el `ProjectProvider` invalida estados de la vista anterior. Proyectos incluye CRUD, archivo/reactivación, resumen con agregaciones reales, membresías `OWNER/ADMIN/EDITOR/VIEWER` y clonación transaccional de configuración sin activos, documentos, ubicaciones, eventos, históricos, notificaciones, ejecuciones ni planos. Código/serie de activo y código de ubicación son ahora únicos por proyecto; la semilla incluye proyectos operativos aislados.
...
- **PROJ-01.1 — estabilización multi-proyecto**: `ProjectScope` pasa de comprobar un rol ad hoc a aplicar capacidades reutilizables en el montaje central de routers: OWNER/ADMIN administran, EDITOR realiza operaciones ordinarias y VIEWER queda en solo lectura (`403` en cualquier escritura). La configuración estructural queda reservada a OWNER/ADMIN. `/api/session` se mantiene global provisional, los usuarios pasan exclusivamente por `/api/projects/:projectId/users` y `/api/users` responde `404`. `ProjectProvider` deja de cargar dos veces el mismo proyecto al almacenar su resultado. Panel, Ubicaciones e Historial recuperan su estado visual protegido con datos deterministas solo en Playwright; no se modifican el HTML, baselines ni el umbral de 0,5 %. La suite E2E reutiliza un contexto Chromium por worker y crea una página aislada por prueba para evitar `net::ERR_NO_BUFFER_SPACE` en Windows.
- **Ubicaciones**: el detalle vuelve a usar su preview acotado de tres activos, incluido en el DTO de detalle, en vez de materializar una página de inventario en el estado de reposo.
