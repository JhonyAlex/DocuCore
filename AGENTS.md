...
**IMG-01 — Soporte de hasta 5 imágenes por activo con carrusel, visor y miniaturas**: FUNCIONAL

- ✅ Hasta 5 imágenes por activo, guardadas en el storage gestionado de DocuCore (`storeDocumentBuffer`/`readDocumentFile`/`removeDocumentFile`); en BD se modela mediante `AssetImage` (`id`, `assetId`, `storageKey`, `mimeType`, `sizeBytes`, `sortOrder`, `createdAt`, migración `20260817110000_asset_images`); retrocompatibilidad total con `ApiAsset.imageUrl` derivado de la primera imagen.
- ✅ API: `POST /api/assets/:id/images` (y alias `/image`) acepta hasta 5 imágenes simultáneas o incrementales (`multipart`, campos `images` o `image`, solo PNG/JPEG/WebP/GIF, máx. 10 MB por archivo; error 400 si se superan 5 fotos); `GET /api/assets/:id/images/:imageId` sirve la imagen inline (`Content-Type` según BD, `Cache-Control: private, max-age=3600`); `GET /:id/image` sirve la primera imagen; `DELETE /api/assets/:id/images/:imageId` elimina una imagen específica y su archivo en disco; `DELETE /:id/image` elimina todas las imágenes; `purge` elimina todos los archivos físicos asociados sin dejar huérfanos.
- ✅ Ficha (`AssetModal`): `AssetImageBox` (`src/components/AssetImageBox.tsx`) muestra la imagen activa con carrusel (flechas ‹ / ›), indicador `1/N` y tira inferior de miniaturas clicables; hover para «Añadir foto» (si < 5) y «Quitar foto» (con `ConfirmDialog`); al hacer clic sobre la imagen abre el visor ampliado.
- ✅ Visor ampliado (`AssetImageViewer.tsx`): modal a pantalla completa (`z-[60]`) con navegación por flechas en pantalla, navegación por teclado (`ArrowLeft` / `ArrowRight`), tira inferior de miniaturas clicables con resaltado de la activa y cierre con `Escape`/`✕`/backdrop sin cerrar la ficha.
- ✅ Alta y edición (`AssetFormModal`): `AssetImagePicker` permite seleccionar hasta 5 imágenes con previsualización de miniaturas locales (blob `URL.createObjectURL`), botón `✕` para descartar fotos individualmente y contador `(N/5)`; las imágenes se suben en lote tras guardar el activo.
- ✅ Validación: lint ✅, typecheck ✅, build ✅, 134 unit tests ✅ y E2E `z-asset-image.spec.ts` adaptado.

**COM-01 — Comentarios en Activos y Documentos**: FUNCIONAL (pendiente de validación manual)

- ✅ Entidad reutilizable `Comment` (`projectId`, `authorId`, `assetId?` XOR `documentId?`, `body`, timestamps) — un comentario pertenece solo a un activo o a un documento, nunca a ambos (validado por API) — con relaciones a `Project`/`User`/`Asset`/`Document` en `onDelete: Cascade` e índices `(projectId, assetId, createdAt DESC, id DESC)`, `(projectId, documentId, createdAt DESC, id DESC)`, `assetId`, `documentId` y `authorId`; migración `20260907120000_comments` (forward-only)
- ✅ Los comentarios **no** forman parte del DTO pesado de `Asset`/`Document`: `GET/POST /api/projects/:projectId/assets/:assetId/comments` y `/documents/:documentId/comments` (+ `/comments/count` ligero para la cabecera de documentos) y `PATCH/DELETE /api/projects/:projectId/comments/:commentId` bajo la política central (`projectScope`); listado paginado por **cursor** `(createdAt DESC, id DESC)` sin OFFSET (20/página, máx. 100, más recientes primero, sin repetir ni perder entre páginas); cursor opaco con `hasMore`/`nextCursor`
- ✅ Permisos con los helpers existentes: VIEWER lee (DTO sin `canEdit/canDelete`); EDITOR+ crea; el **autor** edita/elimina su comentario; **ADMIN/OWNER** (o soporte) gestiona cualquier comentario; archivado (409), plan/suspensión (402/403) y proyecto ajeno (404 por `projectId` en la búsqueda) quedan cubiertos por la política central
- ✅ Validación Zod: cuerpo no vacío tras trim, máx. 4000 caracteres, multilínea; el texto se renderiza siempre como texto (`whitespace-pre-wrap`), solo las URLs http/https se enlazan con `target="_blank" rel="noopener noreferrer"` (`src/lib/comments.ts`); auditoría de alta/edición/borrado (`comment:N`) **sin duplicar el texto** en `AuditLog`
- ✅ `EntityCommentsPanel` (`src/components/comments/`) compartido para activos y documentos: avatar de iniciales, nombre, fecha relativa (exacta en tooltip; `src/lib/time.ts` extraído de NotificationsPopover), «Editado», menú ⋯, edición inline, borrado con `ConfirmDialog`, estados loading/error/vacío/anteriores/creando/editando/eliminando y guarda de secuencia (respuestas obsoletas descartadas); sin refetch del DTO de la entidad al operar
- ✅ Ficha del activo: la pestaña **Resumen** muestra el panel a la derecha (~38 %, desktop, con scroll propio; apilado debajo en tablet/móvil) y carga solo con la ficha abierta y la pestaña visible; **DocumentModal**: acción de cabecera «Comentarios · N» (count ligero) que abre panel lateral colapsable de 360 px (drawer con backdrop en móvil) — al cerrarlo la vista previa recupera su ancho — y la lista se pide solo al abrir el panel; Escape cierra primero la capa de comentarios
- ✅ Validación: lint ✅, typecheck (app + server) ✅, build ✅, `pnpm test` **595/595** ✅ (13 API + 9 unit nuevos: matrices de permisos, cursor sin duplicados, cross-project, papelera/purga, count exacto) y E2E `z-comments.spec.ts` **2/2** ✅; visual sin baselines nuevos (los objetivos `item-modal`/`documents` ya estaban en desfase autorizado y el panel solo aparece con la ficha/drawer abiertos)
...
- ✅ Shell con sesión real: `GET /api/auth/session` (no-store) + `SessionProvider`; las rutas operativas requieren cookie válida. Alta por la UI: el Sidebar se actualiza sin recargar (recarga asíncrona de la sesión). Borrado directo por API (`DELETE /api/assets/:id`): el conteo se actualiza al recargar la página, que es cuando la sesión se vuelve a cargar (E2E verifica ambos)
...
| Comentarios en Activos y Documentos (COM-01) | Implementado y verificado API/E2E |
