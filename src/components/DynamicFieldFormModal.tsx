...
export default function DynamicFieldFormModal({ field, types, busy, error, onClose, onSubmit }: DynamicFieldFormModalProps) {
  ...
  useEffect(() => { setValues(initial(field, types)); setOptionLines(field?.options.map((option) => option.label).join('\n') ?? '') }, [field, types])
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-start justify-center overflow-y-auto bg-slate-900/50 p-4 backdrop-blur-sm" onClick={(event) => event.target === event.currentTarget && !busy && onClose()}>
      <div role="dialog" aria-modal="true" aria-labelledby="dynamic-field-form-title" className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center justify-between border-b border-slate-200 p-5 dark:border-slate-800"><div><div className="text-xs font-mono text-slate-500">{field ? field.key : 'NUEVO CAMPO'}</div><h3 id="dynamic-field-form-title" className="text-lg font-semibold">{field ? 'Editar campo dinámico' : 'Nuevo campo dinámico'}</h3></div><button type="button" onClick={onClose} disabled={busy} aria-label="Cerrar" className="rounded-lg p-2 hover:bg-slate-100 dark:hover:bg-slate-800">×</button></div>
        <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col">
          <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5 scrollbar-thin">
            ...
            <section><h4 className="mb-3 text-sm font-semibold">Información básica</h4><div className="grid grid-cols-1 gap-4 md:grid-cols-2">
