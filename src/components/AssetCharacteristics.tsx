...
export default function AssetCharacteristics({ asset, onChanged }: AssetCharacteristicsProps) {
  ...
  const beginEdit = () => {
    setValues(Object.fromEntries(fields.map((field) => [field.definitionId, field.value])))
    setError(null)
    setEditing(true)
  }
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              {groupFields.map((field) => (
                <div key={field.definitionId} className="rounded-lg bg-slate-50 p-3 dark:bg-slate-800/50">
                  <div className="mb-1 flex items-start justify-between gap-2"><label className="text-xs text-slate-500 dark:text-slate-400">{field.fieldName}{field.required && <span className="ml-1 text-red-500">*</span>}</label>{field.periodicity && <span className="rounded-full bg-brand-50 px-2 py-0.5 text-[10px] font-medium text-brand-700 dark:bg-brand-900/30 dark:text-brand-300">{field.periodicity} · {field.periodicityMode === 'Calendario' ? 'Calendario' : 'Realización'}</span>}</div>
                  {editing ? <DynamicFieldInput field={field} value={values[field.definitionId]} onChange={(value) => setValues((current) => ({ ...current, [field.definitionId]: value }))} disabled={saving} /> : <div className={`text-sm font-medium ${displayValue(field) === 'Sin informar' ? field.required ? 'text-amber-600 dark:text-amber-400' : 'text-slate-400' : ''}`}>{displayValue(field)}</div>}
                  {field.description && <p className="mt-1 text-xs text-slate-400">{field.description}</p>}
                  {!editing && field.fieldType === 'DATE' && field.periodicity && Boolean(field.value) && <button type="button" onClick={() => { setCompleteError(null); setCompleting(field) }} className="mt-2 text-xs font-medium text-brand-600 hover:text-brand-700 dark:text-brand-400">Completar y programar siguiente →</button>}
                </div>
              ))}
            </div>
          </section>
