...
export default function ItemModal({ item, onClose }: { item: Item | null; onClose: () => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
              <div className="md:col-span-2 grid grid-cols-2 gap-3">
                <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                  ...
                  <div className="mt-1"><StatusChip label={item.status} chipClass={item.statusChipClass} pulseDot={item.pulseDot} /></div>
                </div>
                <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                  <div className="text-xs text-slate-500">Tipo</div>
                  <div className="mt-1 text-sm font-medium">{item.type}</div>
                </div>
                <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                  <div className="text-xs text-slate-500">Ubicación</div>
                  <div className="mt-1 text-sm font-medium">{item.location}</div>
                </div>
                <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                  <div className="text-xs text-slate-500">Responsable</div>
                  <div className="mt-1 text-sm font-medium flex items-center gap-1.5">
                    <span className={`w-5 h-5 rounded-full ${item.responsibleColor} text-white text-[10px] font-medium flex items-center justify-center`}>{item.responsibleInitials}</span>
                    {item.responsible}
                  </div>
                </div>
                <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                  <div className="text-xs text-slate-500">Nº de serie</div>
                  <div className="mt-1 text-sm font-mono">{item.serialNumber}</div>
                </div>
                <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                  <div className="text-xs text-slate-500">Instalación</div>
                  <div className="mt-1 text-sm font-medium">{item.installDate}</div>
                </div>
              </div>
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <button className="text-sm text-red-600 hover:text-red-700">Dar de baja</button>
          <div className="flex items-center gap-2">
            ...
            <button className="px-3 py-2 rounded-lg bg-brand-600 hover:bg-brand-700 text-white text-sm font-medium">Editar</button>
          </div>
        </div>
      </div>
    </div>
