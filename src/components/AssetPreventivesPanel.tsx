    ...
    <section className="space-y-4">
      ...
      {showAssignForm && (
        <div className="rounded-xl border border-brand-200 bg-brand-50/60 p-4 dark:border-brand-900/50 dark:bg-brand-950/30">
          ...
          <div className="mt-3 flex justify-end gap-2"><button type="button" onClick={() => setShowAssignForm(false)} disabled={busy} className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs dark:border-slate-700 dark:bg-slate-800">Cancelar</button><button type="button" onClick={() => void withUpdate(() => createAssetPreventive(asset.projectId, asset.id, { planId: selectedPlanId, scheduledDate }), 'No se pudo asignar el plan preventivo.').then((ok) => ok && setShowAssignForm(false))} disabled={busy || !selectedPlanId || !scheduledDate} className="rounded-lg bg-brand-600 px-3 py-1.5 text-xs font-medium text-white disabled:opacity-40">{busy ? 'Asignando…' : 'Confirmar asignación'}</button></div>
        </div>
      )}
