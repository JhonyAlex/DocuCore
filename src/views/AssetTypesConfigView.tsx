  ...
  return (
    <section className="fade-in">
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
        {loading ? <div className="p-8 text-center text-sm text-slate-500">Cargando tipos…</div> : error ? <div role="alert" className="p-8 text-center text-sm text-red-600">{error}</div> : types.length === 0 ? <div className="p-8 text-center"><p className="text-sm text-slate-500">No hay tipos de activo configurados.</p><button type="button" onClick={() => setFormType(null)} className="mt-2 text-sm font-medium text-brand-600">Crear el primero</button></div> : <div className="overflow-x-auto"><table className="w-full text-sm"><thead className="border-b border-slate-200 bg-slate-50 text-xs text-slate-500 dark:border-slate-800 dark:bg-slate-800/50"><tr><th className="w-10 px-4 py-3"><input type="checkbox" aria-label="Seleccionar todos los tipos" checked={selection.allSelected(activeIds)} ref={(node) => { if (node) node.indeterminate = selection.someSelected(activeIds) }} onChange={() => selection.toggleAll(activeIds)} /></th><th className="px-4 py-3 text-left">Tipo de activo</th><th className="px-4 py-3 text-left">Estado</th><th className="px-4 py-3 text-left">Activos</th><th className="px-4 py-3 text-left">Campos dinámicos</th><th className="w-14 px-4 py-3" /></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-800">{types.map((type) => <tr key={type.id} className={type.isActive === false ? 'opacity-55' : ''}><td className="px-4 py-3"><input type="checkbox" aria-label={`Seleccionar ${type.name}`} disabled={type.isActive === false} checked={selection.isSelected(type.id)} onChange={() => selection.toggle(type.id)} /></td><td className="px-4 py-3"><div className="flex items-center gap-2 font-medium"><span className="flex h-7 w-7 items-center justify-center rounded-md bg-brand-50 text-brand-600 dark:bg-brand-900/30 dark:text-brand-300"><AssetIcon iconKey={type.iconKey} size={16} /></span>{type.name}</div><div className="text-xs text-slate-400">Tipo {type.id} · {type.iconKey}</div></td><td className="px-4 py-3"><span className={`rounded-full px-2 py-1 text-xs ${type.isActive === false ? 'bg-slate-100 text-slate-500 dark:bg-slate-800' : 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300'}`}>{type.isActive === false ? 'Archivado' : 'Activo'}</span></td><td className="px-4 py-3 text-xs">{type.assetCount ?? 0}</td><td className="px-4 py-3 text-xs">{type.fieldCount ?? 0}</td><td className="px-4 py-3"><RowActionsMenu ariaLabel={`Acciones de ${type.name}`} items={[{ label: 'Editar', onSelect: () => { setFormError(null); setFormType(type) } }, ...(type.isActive === false ? [{ label: 'Reactivar', variant: 'success' as const, onSelect: () => void reactivate(type) }] : [{ label: 'Archivar', variant: 'danger' as const, onSelect: () => setArchiveIds([type.id]) }])]} /></td></tr>)}</tbody></table></div>}
      </div>
