...
export default function DynamicFieldsConfigView() {
  ...
  return (
    <section className="fade-in">
      <div className="mb-6 flex items-end justify-between gap-4"><div><button type="button" onClick={() => navigate('/config')} className="mb-2 text-xs font-medium text-brand-600">← Configuración</button><h1 className="text-2xl font-semibold tracking-tight">Campos dinámicos</h1><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Características personalizadas del proyecto {session?.project.name ?? ''}</p></div><button type="button" onClick={() => { setFormError(null); setFormField(null) }} className="rounded-lg bg-brand-600 px-3 py-2 text-sm font-medium text-white hover:bg-brand-700">Nuevo campo</button></div>
      <div className="mb-4 flex flex-wrap items-center gap-3"><select aria-label="Filtrar por tipo de activo" value={typeFilter} onChange={(event) => { setTypeFilter(Number(event.target.value)); selection.clear() }} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-900"><option value="0">Todos los tipos de activo</option>{types.map((type) => <option key={type.id} value={type.id}>{type.name}</option>)}</select><label className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300"><input type="checkbox" checked={showInactive} onChange={(event) => { setShowInactive(event.target.checked); selection.clear() }} />Mostrar archivados</label></div>
