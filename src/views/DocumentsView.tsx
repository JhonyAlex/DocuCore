...
import { formatApiDate, formatDocumentSize } from '@/lib/itemMappers'
...
export default function DocumentsView() {
  ...
  return <section className="fade-in">
    ...
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 text-xs uppercase"><tr><th className="text-left px-4 py-3">Documento</th><th className="text-left px-4 py-3">Activos asociados</th><th className="text-left px-4 py-3">Tipo</th><th className="text-left px-4 py-3">Versión</th><th className="text-left px-4 py-3">Emisión</th><th className="text-left px-4 py-3">Vencimiento</th><th className="text-left px-4 py-3">Estado</th><th className="text-right px-4 py-3">Acciones</th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-800">
      ...
      {!loading && !error && documents.map((document) => { const version = document.currentVersion; const format = version?.originalName.split('.').pop()?.toUpperCase() ?? 'DOC'; return <tr key={document.id} onClick={() => setEditing(document)} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 cursor-pointer"><td className="px-4 py-3"><div className="flex items-center gap-3"><div className={`w-9 h-9 rounded-lg ${documentIconClasses[document.type] ?? documentIconClasses.Manual} flex items-center justify-center text-xs font-bold`}>{format}</div><div><button type="button" onClick={(event) => { event.stopPropagation(); setEditing(document) }} className="font-medium text-left hover:text-brand-600">{document.name}</button><div className="text-xs text-slate-500">{version ? `${formatSize(version.sizeBytes)} · Subido ${formatApiDate(version.uploadedAt)}` : 'Sin versiones'}</div></div></div></td><td className="px-4 py-3 text-slate-600 dark:text-slate-300">{document.items.length > 0 ? document.items.map((item) => `${item.code} · ${item.name}`).join(', ') : '—'}</td><td className="px-4 py-3"><span className={`chip ${typeClasses[document.type] ?? typeClasses.Manual}`}>{document.type}</span></td><td className="px-4 py-3 font-mono text-xs">{version ? `v${version.version}` : '—'}</td><td className="px-4 py-3 text-xs">{version ? formatApiDate(version.issueDate) : '—'}</td><td className="px-4 py-3 text-xs">{version?.expiryDate ? formatApiDate(version.expiryDate) : <span className="text-slate-400">—</span>}</td><td className="px-4 py-3"><span className={`chip ${statusClasses[document.status]}`}>{document.status}</span></td><td className="px-4 py-3 text-right"><button type="button" onClick={(event) => { event.stopPropagation(); void downloadDocument(document.id) }} className="px-2 py-1 rounded hover:bg-slate-200 dark:hover:bg-slate-700 text-xs">Descargar</button></td></tr> })}
    </tbody></table></div></div>
