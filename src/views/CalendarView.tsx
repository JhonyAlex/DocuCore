...
export default function CalendarView() {
  ...
  return <section className="fade-in">
    ...
    <CalendarEventDetails event={selected} busy={busy} error={operationError} onClose={closeLayers} onComplete={() => void complete()} onEdit={() => { if (!selected) return; setOperationError(null); setForm({ title: selected.title, date: selected.date, category: selected.category, assetId: selected.assetId, assetLabel: selected.asset ? `${selected.asset.code} · ${selected.asset.name}` : undefined }) }} onDelete={() => setConfirmDelete(selected)} onOpenAsset={() => { if (selected?.assetId) navigate(`/projects/${projectId}/assets?assetId=${selected.assetId}`) }} onOpenDomain={() => navigate(selected?.source === 'document' ? `/projects/${projectId}/docs` : `/projects/${projectId}/assets?assetId=${selected?.assetId ?? ''}${selected?.source === 'preventive' ? `&preventiveExecutionId=${selected.sourceId}` : ''}`)} />
